...
async def Buy_month(call: types.CallbackQuery):
    ...
    if payment_info is None:
        ...
        bill_id = f"{str(int(time.time() * 100))[4:]}{int(random.random() * 1000)}"

        array = {
            'amount': Month_count * CONFIG['one_month_cost'],
            'payment': bill_id,
            'shop': 6285,
            'currency': 'RUB',
            'desc': f'Оплата VPN на {Month_count} мес. для пользователя {call.from_user.id}',
            'secret': CONFIG['secret_key']
        }
        string_to_hash = '|'.join(str(value) for value in array.values())
        sign = hashlib.md5(string_to_hash.encode()).hexdigest()
        url_to_pay = f'https://payok.io/pay?amount={array["amount"]}&payment={array["payment"]}&&shop=6285&currency=RUB&desc={array["desc"]}&sign={sign}'
        await user_dat.NewPay(bill_id, Month_count * CONFIG['one_month_cost'], Month_count * 2592000, call.message.id,
                              url_to_pay)

        Butt_payment = types.InlineKeyboardMarkup()
        Butt_payment.add(
            types.InlineKeyboardButton(e.emojize("Оплатить :money_bag:"), url=url_to_pay))
        Butt_payment.add(
            types.InlineKeyboardButton(e.emojize("Отменить платеж :cross_mark:"),
                                       callback_data=f'Cancel:' + str(user_dat.tgid)))
    ...
    while True:
        try:
            ...
            if len(log) > 0:
                ...
                param = {'API_ID': CONFIG['API_ID'],
                         'API_KEY': CONFIG['API_KEY'],
                         'shop': 6285}
                payments = requests.post(url=URL, data=param).json()
                all_pay = {}
